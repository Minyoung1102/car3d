const second_a = document.getElementById("second_a");

function firstDesign3() {
  function handleColorClick(event) {
    const color = event.target.id;
    if (color == "second_white") {
      model.children[0].children[0].material = new THREE.MeshStandardMaterial({
        color: 0x8a8b8e,
      });
    } else if (color == "second_black") {
      model.children[0].children[0].material = new THREE.MeshStandardMaterial({
        color: 0x111111,
      });
    } else if (color == "second_gray") {
      model.children[0].children[0].material = new THREE.MeshStandardMaterial({
        color: 0x444040,
      });
    } else if (color == "second_blue") {
      model.children[0].children[0].material = new THREE.MeshStandardMaterial({
        color: 0x0f2375,
      });
    } else if (color == "second_red") {
      model.children[0].children[0].material = new THREE.MeshStandardMaterial({
        color: 0x770a13,
      });
    }
  ...
  var canvas = document.querySelector("#second canvas");

  secondSection.removeChild(canvas);
...
second_a.addEventListener("click", firstDesign3);
